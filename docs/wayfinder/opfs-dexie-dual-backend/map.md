...
- Now that the abstraction shape is settled, the remaining piece of how
  `sync-tracker-actors.ts`/`sync-metadata-actors.ts` and the program-rule
  engine (`executeProgramRules` in `utils/utils.ts`) actually get wired to
  call through it (vs. `SqlDriver` directly today) is mostly mechanical —
  not sharp enough yet to say whether it needs its own ticket or falls out
  naturally while implementing tickets 002/003/004.
