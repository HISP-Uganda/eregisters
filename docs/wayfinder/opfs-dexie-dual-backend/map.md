...
- **`utils.ts`'s cascade-delete/resend helpers remain SQL-only** — `deleteEventWithChildren`/`deleteTrackedEntityWithChildren`/`resendEventWithChildren` (`src/utils/utils.ts`) still call `getSqlDriver()` and SQL-only row-adapters directly, unaffected by the sync.ts/App.tsx wiring above (they're UI-triggered, not part of the sync machine). On the Dexie backend these will fail today — a real gap against the "full feature parity" destination, not yet sharp enough to ticket (does it need its own `TrackerCollectionUtils`-based rewrite, or a `collections.ts`-style dispatcher?).
- **Reverse migration on backend switch isn't wired to execute** — `device-storage-settings.tsx` already documents this scope cut; now that App.tsx actually branches on the resolved backend, wiring an explicit "switch and migrate" action through `migrate-from-sqlite.ts` is unblocked but not yet specified.
