...
- [How Do We Verify SW-Level Timeout/5xx Handling Pre-Deploy?](tickets/005-sw-verification-strategy.md) — a tiny stand-in Node `http` server (no Express needed) serving a real `pnpm build` output, driven by a one-off headless-Chrome spike via the `mcp__claude-in-chrome` tools (matching the Dexie-to-SQLite migration's own ticket 001 precedent — no Playwright added). **Run and passed** against the real patched `service-worker.js`: a 503 on a cached asset returns the cached content (not the 503); a hung asset request resolves in 8003.7ms (matching `networkTimeoutSeconds:8` almost exactly); a hung navigation resolves with the real precached `index.html`, bounded, not stuck. `clients.claim()` (patch 1) also reconfirmed working. Spike lived in scratch space only, nothing committed.
